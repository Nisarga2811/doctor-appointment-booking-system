<?php
    ...
    if(isset($_POST['doc_login']))
    {
      ...
      $stmt=$mysqli->prepare("SELECT email ,password , doc_id FROM doc_logs WHERE email=? and password=? ");//sql to log in user
      $stmt->bind_param('ss',$email,$password);//bind fetched parameters
      $stmt->execute();//execute bind
      $stmt -> bind_result($email,$password,$doc_id);//bind result
      $rs=$stmt->fetch();
      $_SESSION['doc_id']=$doc_id;//assaign session to passenger id
			
      if($rs)
      {//if its sucessfull
        header("location:doctors\doc-index.php");
      }
                        ...
                        <form method="POST">
                            ...
                            <div class="form-group row mt-2">
                                <div class="col-md-4"></div>
                                    <div class="col-md-8">
                                    <div class="col-6"><input type="submit" name ="doc_login" class="btn btn-success btn-xl" value ="Log In"></div>
                                       </div>
                                </div>
                        </form>
                    </div>
